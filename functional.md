...
有了函数，我们应该给它起个名字，这样就不用啰嗦地再写一大堆参数和逻辑关系了。这就完成了函数的定义。想用它的时候，我们写下这个函数名，再指明其对应的几个参数是什么，（那几个参数可能是其他基本符号的组合，甚至是其他函数，那时我们就递归地为那些函数赋值），通过几个旧基本符号就得到了几个新基本符号。这就是函数的赋值。使人困惑的是，这个函数名也成了一个变量，在定义了它之后它也被约束了。事实上，为函数或表达式起名字的过程也可以理解成定义一个函数。在之后的解释器一节中会更深入地讨论赋值问题，尤其是由于函数参数和”起名字“导致的约束变量代入问题。
