...
从这个角度考虑，如果函数式编程中的函数只有两个基本表达式可以使用：Forall和Inductive（前者是定义，后者是归纳定义），那么任何函数式编程的函数（程序）都是一个证明。它想证明的逻辑结论可以直观地从该函数的类型看出来：一个类型P就是一个命题P，而蕴含”->“就是函数的调用。我们还可以从更深层次认识证明和函数程序的对应关系：给这个函数体赋值的过程就是化简证明的过程。
