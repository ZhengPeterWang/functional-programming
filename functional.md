...
命令式编程是这么想的：找两个内存，一个写上1，一个写上2，再找一个空内存。找一个加法二极管，把1和2输入，把输出的结果放进第三个内存，把前两个内存删掉，把第三个内存存起来。等号代表着赋值。

函数式编程是这么想的：有1，2，3三个同属自然数集的元素，定义一个二元映射(+)，1和2的笛卡尔积在它的下面映射为3。等号代表着映射。
...
定义函数，就是定义逻辑关系。巧妇难为无米之炊，逻辑关系需要抽象材料的支撑，这“米”就是变量。变量可以取它应该属于集合的任何元素，而我们可以用符号表达变量之间的关系。let f a b c = a + b + c 从纯抽象的角度，就是定义了(a, b, c)与a+b+c的值的映射。当然这也可以理解为a与fun b c -> a + b + c的映射，在后一个函数里a就不再是自由变量，就被量词约束了，因此它也就可以看成是被赋值了。也就是说多元函数不仅可以看成笛卡尔积的函数，也可以看成一元函数的一元函数，这种多元函数与高阶函数的对应关系称为Currying。

函数中的变量称为函数的参数。在函数体中可以随意使用参数，但要注意的是，在函数体中这些参数名就被约束(bounded)了。函数体中如有自由变量(free variable)，则不能与这些被约束变量起相同的名字。这是后话。在函数式编程中一般不允许有自由变量（无法赋值变量）存在，也即变量必须是函数的参数。

以上我们定义的函数没有名字，只是描述了一个变量和另一个变量的关系，这种函数叫做匿名函数(anonymous function)。但更多的时候我们会给这个函数起个名字，这样就不用啰嗦地再写一大堆参数和逻辑关系了。这可以理解为在全局中用一个变量代指这个函数。想用它的时候，我们写下这个变量，就找到了这个函数。再指明其对应的几个参数是什么，（那几个参数可能是其他基本符号的组合，甚至是其他函数，那时我们就递归地为那些函数赋值），就可以完成函数的计算，最终通过几个旧基本符号就得到了几个新基本符号。这就是函数的赋值。在之后的解释器一节中会更深入地讨论赋值问题，尤其是由于函数参数导致的约束变量代入问题。

特别要提到的情形是函数的定义中调用了自己所代指的变量的情形，这就是递归。递归栈是如何处理这种函数关系的解决办法，尽管函数式编程并不关心栈的细节。在函数式编程中这就是实现循环的方式。事实上，任何一个循环都可以转化成一个指明有几个变量要存起来的尾递归。尾递归优化可以避免递归层数过多导致的堆栈溢出(stack overflow)。
## 函数的类型集合与数据结构
基本符号不能脱离其定义的集合而存在，而我们不想把不同类型的符号（属于不同集合的符号）混起来。这样我们就有了符号类型（集合）的概念。基本符号的类型井水不犯河水，但我们可以去定义新的类型，把已有的不同类型加起来（variants）或者乘起来（tuples），构建一些新的集合，这就是加法类型和乘法类型作为代数数据类型的表现。集合间的运算可以反射到基本符号间的运算，只不过这是定义新的基本符号的形式。

有的函数并不关心类型，这样的函数需要多态。'a就是表达多态的工具。

通过type constructor，我们可以以加法类型和乘法类型为基础构造非常丰富的类型。list就是使用了type加法和乘法的type constructor。需要注意的是，构造新类时可以使用自己构造，这就是递归（归纳）定义。
...
对于把大量数据组合起来的数据结构类型，我们很容易定义对每一个单个对象作用的函数，却对大对象束手无策。这时就可以用函数的函数为每个对象量身定制一套把其中每一个元素都映射到其他对象的函数。map和fold就是两个典型例子。map通过接受一个把小对象映射到另一个小对象的映射，把大对象映射到另一个大对象；fold把大对象映射到一个反映大对象性质的小元素（当然未必，也可能是另一个大对象）。任何数据结构都可以有map和fold。
...
面向对象编程的重要思想就是构建一类有自己变量和方法的对象。struct可以实现它，不过你得造一个自己的数据类型，用加起来和乘起来造出一个数据类型，然后限定这个struct的方法（函数）只可以用在你定义的类型上，这就等效于实现了类。把实施细节藏起来，也把你那自定义的数据类型藏了起来，外面看不出这个数据类型和原本的实现中的基本类型有什么关系，这就是抽象性原理的体现。略麻烦一点的是，为每一个struct，现在你必须定义一个t，作为专属于它的对象。
...
### 无穷数据结构 流 延迟赋值
流是没有递归起点的递归数据结构。但OCaml 编译器的赋值是立即型(eager)：一遇到基本符号，它就想为它赋值（不同语言不同，比如Haskell就不是，除非你想知道它的值，它就是不去赋值）。于是无穷数据结构毫无疑问地将会走向堆栈溢出。怎么办呢？考虑到函数也是一种基本符号，可以让函数作为阻断赋值的工具，即创建一个以unit为参数的dummy函数，让它取代原来的递归，OCaml遇到函数就置之不理了，这样就能成功创建流了。这种结构称为thunk。当然OCaml像Haskell一样也支持延迟赋值，那时就需要使用Lazy模块。
...
引用与函数（闭包）作为基本符号还有一个重要的作用：实现递归。先定义一个函数的引用，再修改引用使它在函数中引用自己即可构造无穷递归。

### 并发和Promise
操作系统在工作时，假定它需要去完成的任务都是相互独立的，这样它就可以这一秒干干这个，后一秒再干干那个，以防止某个应用逗留太久，这就是并发编程。操作系统默认执行并发，然而，有时我们需要让某段代码与另一段代码同时运行互不干扰；有时又要强令某段代码必须于另一段代码后执行，即*杜绝*并发现象。例如IO，不输入就没东西输出；还有时我们需要强令某段代码必须在另一段代码执行后（或者是某一现象发生后）立即执行，也就是在那段代码后缀一段必须顺序执行的代码，这就是Listener Pattern。这些现象需要异步输入输出(asynchronous I/O)。

在OCaml和其他函数式编程语言中，Promise是实现非并发输入输出的工具，实现它的工具包是Lwt。Promise就是一张空头支票，意味着这个空会被填上，但我现在不一定填它，而其他事情可以与填好这个promise同时完成。从这个角度讲Promise是一种懒惰赋值，因此解决promise的过程就与定义promise的过程分开了，这样程序定义promise之后就可以去在计算它的同时干其他事情了。Promise这张空头支票还可以被送给其他函数。实现这个现象的机制是bind。bind让后一个函数变成前一个函数的callback。前一个函数的输出值是Promise，它会在不确定的未来被解决。而callback只能在这张空头支票兑现之后执行，这样就在这个层次上杜绝了并发现象。bind函数是一种特殊的通过旧空头支票a制造新空头支票b的函数，它先造出一张空头支票b，b的性质是旧空头支票a兑现时必须立即运行callback函数，生出一个新的空头支票c，而空头支票c与空头支票b一模一样——c解决的同时b也必须被解决。这样b在a解决之后就立即被解决，保证IO不会出现混乱。使用bind函数可在promise带来的并发中实现顺序执行。

Promise的最大特点是在把生成结果和处理结果的代码完整分离的情况下还能够实现两者并发或顺序执行的控制。
...
实现赋值有两种基本方法：小步子和大步子。小步子的特点是每次递归都只执行一次赋值（注意不是代入变量！），即一小步。小步子只在一个函数(step)里实现递归，因而便于深入分析运算的性质（任何一个自恰的形式系统必须满足：（1）一个表达式要么可以走一小步要么已经计算完毕；（2）走一小步不改变表达式的类型），但写起来很麻烦。大步子的特点是大量地使用递归，每个赋值函数都可以交叉地使用递归，没有step函数，但有一些eval函数，一层递归可以执行大量的赋值。大步子是大多数解释器采纳的方式。
...
在环境模型中，特别需要注意的是在函数语句中出现非本函数参数变量时，这个变量名应该在哪个作用域中寻找。静态作用域（词法作用域,lexical scope）认为这个变量名应该在函数定义时的环境中寻找，而动态作用域(dynamical scope)则认为这个变量名应在函数调用时的环境中寻找。静态作用域是目前除了Lisp等少数语言外大多数语言的选择。需要指出的是，动态作用域能很自然地由环境模型实现，而源自代入模型的静态作用域为了被环境模型实现，必须改变环境的数据结构，为其中所有的函数加上*其定义时的环境*，这种包括函数定义环境的存储函数的结构称为闭包(closure)。
## 附录：形式语言和自动机
